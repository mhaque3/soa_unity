...
using System.Linq;
using Gamelogic.Internal.BinaryHeap;
using UnityEngine;
    ...
    public static partial class Algorithms
    {
        ...
        */
		...
		/** 
			...
			@param neighborToNeighborCost The actual cost of reaching one node from 
		...
		*/
            ...
            where TPoint : IGridPoint<TPoint>
        {
            ...
            while (!openSet.IsEmpty())
            {
                var current = FindNodeWithLowestScore2(openSet, fScore);
	...
	where TPoint : IGridPoint<TPoint>
       ...
       */
			...
			where TPoint : IGridPoint<TPoint>
		{
			...
			while (!openSet.IsEmpty())
			{
				...
				var current = FindNodeWithLowestScore2(openSet, costToReach);
				openSet.Remove(current);
				closedSet.Add(current);
       ...
       */
        ...
        */
        ...
        #region Helpers
        private static TPoint FindNodeWithLowestScore2<TPoint>(IList<TPoint> list, IDictionary<TPoint, float> scoreTable)
        {
	        var minItem = list[0];
		...
		private static TPoint FindNodeWithLowestScore2<TPoint>(IList<TPoint> list, IDictionary<TPoint, int> scoreTable)
		{
			var minItem = list[0];
			var minScore = scoreTable[minItem];
