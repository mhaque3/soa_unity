...
// Note:
// If running within Unity, replace all System.WriteLine( with Debug.Log(

namespace soa
{
    using Hashtable = ExitGames.Client.Photon.Hashtable; // Uncomment this if running in Unity

    class PhotonCommManager : IPhotonPeerListener
    {
        ...
        // Constructor
        public PhotonCommManager(DataManager dataManager, Serializer serializer,
        ...
        public void start()
        {
            ...
            lock (startEligibleMutex)
            {
                ...
                if (startEligible)
                {
                    ...
                    Debug.Log("PhotonCommManager: Update thread started");
                }
                else
                {
                    // Photon update thread already running, no need to do anything else
                    Debug.Log("PhotonCommManager: start() has no effect, update thread already running");
                }
            } // Unlock
        }
        ...
        public void terminate()
        {
            ...
            lock (startEligibleMutex)
            {
                ...
                if (!startEligible)
                {
                    ...
                    Debug.Log("PhotonCommManager: Termination request sent");
                ...
                else
                {
                    ...
                    Debug.Log("PhotonCommManager: terminate() has no effect, update thread already inactive");
                }
            } // Unlock
        }
        ...
        private void photonUpdate()
        {
            ...
            while (currState != State.TERMINATED)
            {
                ...
                {
                    Debug.Log("PhotonCommManager: " +
                        getStateString(prevState) + " -> " +
                        getStateString(currState));
                }
                    ...
                    case State.DISCONNECTED: // No connections exist and no pending connections
                        {
                            ...
                            else
                            {
                                ...
                                Debug.Log("PhotonCommManager: Connecting to Photon server at " +
                                    ipAddress);
                    ...
                    case State.CONNECTED: // Connected to the server, no request to join room sent yet
                        {
                            ...
                            else
                            {
                                ...
                                Debug.Log("PhotonCommManager: Joining room \"" + roomName + "\"");
        ...
        public void OnOperationResponse(OperationResponse operationResponse)
        {
            switch (operationResponse.OperationCode)
            {
                case LiteOpCode.Join:
                    ...
                    Debug.Log("PhotonCommManager: Joined room \"" + roomName + "\" as Player " + mActorNumber);
                    break;
                ...
                case 0: // Received a Protobuff message
                    {
                        ...
                        else
                        {
                            ...
                            Debug.Log("PhotonCommManager: Belief deserialization failed");
                        }
                        break;
                    }
                case LiteEventCode.Join:
                    {
                        ...
                        if(actorNrJoined != mActorNumber){
                            Debug.Log("PhotonCommManager: Player " + actorNrJoined + " joined the room");
                        }

                        // Provide update on all players in room
                        int[] actorList = (int[])eventData.Parameters[LiteEventKey.ActorList];
                        if(actorList.Length > 0){
                            string statusMessage = "PhotonCommManager: Players currently in room {" + actorList[0];
                ...
                case LiteEventCode.Leave:
                    {
                        ...
                        int actorNrJoined = (int)eventData.Parameters[LiteEventKey.ActorNr];
                        Debug.Log("PhotonCommManager: Player " + actorNrJoined + " left the room");
        ...
        public void addOutgoing(Belief b)
        {
            ...
            else
            {
                ...
                Debug.Log("PhotonCommManager: Belief serialization failed");
            }
        }
        ...
        {
            // Debug.Log("PhotonCommManager: DebugReturn() " + message); 
        }
