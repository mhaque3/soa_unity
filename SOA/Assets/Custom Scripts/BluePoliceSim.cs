...
public class BluePoliceSim : MonoBehaviour 
{
    ...
    public void assignTask()
    {
        ...
        else
        {
            ...
            // Pick one agent with civilian to pursue
            ...
            foreach (int id in redUnitDatabase.Keys)
            {
                ...
                if (redUnitInfo.isPursueCandidate && redUnitInfo.hasCivilian)
                {
            ...
            // Choose from those without civilians if we didn't find a pursue target
    ...
    public GameObject assignNewProtectedSite(GameObject currPatrolTarget){
        ...
        for(int i=0; i<protectedSites.Count; i++)
        {
            ...
            {
                // Weight
                weight[i] = protectedSiteVirtualCasualties[p.name] / GetRangeTo(p);
            }
        ...
        float accum = 0.0f;
        GameObject chosenSite = protectedSites[0];
        for(int i=0; i<protectedSites.Count; i++)
        {
            accum += weight[i];
            if (randEval <= accum)
